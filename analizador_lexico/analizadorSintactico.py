import analizador_lexico.ply.yacc as yacc
import os
import codecs
import re
from analizador_lexico.analizadorLexico import tokens
from sys import stdin
from analizador_lexico.analizadorSemantico import *
precedence = (
...
def p_constAssignmentList1(p):
	'''constAssignmentList : ID EQUAL NUMBER'''
	p[0] = constAssignmentList1(Id(p[1]),Equal(p[2]),Number(p[3]),"constAssignmentList1")
	#print("constAssignmentList 1")

def p_constAssignmentList2(p):
	'''constAssignmentList : constAssignmentList COMMA ID EQUAL NUMBER'''
	p[0] = constAssignmentList2(p[1],Id(p[3]),Equal(p[4]),Number(p[5]),"constAssignmentList2")
...
def p_relation1(p):
	'''relation : EQUAL'''
	p[0] = relation1(Equal(p[1]),"relation1")
	#print "relation 1"
...
def p_multiplyingOperator1(p):
	...
	p[0] = multiplyingOperator1(Multi(p[1]),"multiplyingOperator")
...
def intern_test(directorio=None):
    if directorio == None:
    	directorio = 'test/'
